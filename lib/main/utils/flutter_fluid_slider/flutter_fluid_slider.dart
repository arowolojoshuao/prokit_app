...
class _FluidSliderState extends State<FluidSlider> with SingleTickerProviderStateMixin {
  ...
  TextStyle _currentValTextStyle(BuildContext context) {
    final TextStyle defaultStyle =
        widget.showDecimalValue ? Theme.of(context).textTheme.subtitle1!.copyWith(fontWeight: FontWeight.bold) : Theme.of(context).textTheme.headline1!.copyWith(fontWeight: FontWeight.bold);
...
class _ThumbSplashPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (showContact!.value >= 0.5) {
      ...
      path.quadraticBezierTo(center.dx, size.height, thumbPadding / 2, center.dy);

      path.lineTo(size.width - thumbPadding / 2, center.dy);

      path.quadraticBezierTo(center.dx, size.height, size.width + 0.0, size.height + 6.0);
