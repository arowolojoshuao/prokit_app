class ClusteringHelper {
      ...
      if (database != null) {
        ...
        final listBounds = list.where((p) {
          ...
          final bool latQuery = (leftTopLatitude > rightBottomLatitude)
              ? p.location.latitude <= leftTopLatitude && p.location.latitude >= rightBottomLatitude
              : p.location.latitude <= leftTopLatitude || p.location.latitude >= rightBottomLatitude;

          final bool longQuery = (leftTopLongitude < rightBottomLongitude)
              ? p.location.longitude >= leftTopLongitude && p.location.longitude <= rightBottomLongitude
              : p.location.longitude >= leftTopLongitude || p.location.longitude <= rightBottomLongitude;
  ...
  List<AggregatedPoints> _retrieveAggregatedPoints(List<LatLngAndGeohash> inputList, List<AggregatedPoints> resultList, int level) {
    assert(() {
    ...
    for (var i = 0; i < aggregation.length; i++) {
      ...
      if (a.count == 1) {
        if (bitmapAssetPathForSingleMarker != null) {
          bitmapDescriptor = await BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(48, 48)), bitmapAssetPathForSingleMarker);
        } else {
          bitmapDescriptor = BitmapDescriptor.defaultMarker;
        }
      } else {
        // >1
        final Uint8List markerIcon = await getBytesFromCanvas(a.count.toString(), getColor(a.count));
  ...
  updatePoints(double zoom) async {
    ...
    try {
      ...
      if (database != null) {
        listOfPoints = await DBHelper.getPoints(database: database, dbTable: dbTable, dbLatColumn: dbLatColumn, dbLongColumn: dbLongColumn, whereClause: whereClause);
      } else {
        listOfPoints = list;
      }
      ...
      final Set<Marker> markers = listOfPoints.map((p) {
        ...
        return Marker(
          markerId: markerId,
          position: p.location,
          infoWindow: InfoWindow(title: "${p.location.latitude.toStringAsFixed(2)},${p.location.longitude.toStringAsFixed(2)}"),
          icon: bitmapAssetPathForSingleMarker != null ? BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(48, 48)), bitmapAssetPathForSingleMarker) : BitmapDescriptor.defaultMarker,
        );
      }).toSet();
      updateMarkers(markers);
    } catch (ex) {
